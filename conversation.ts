...
const conversations = new Map<
  string,
  { parentMessageId: string; timestamp: Date }
>();
...
export async function doConversation(
  ...
  try {
    if (
      await shouldContinueConversation(
        question,
        conversationId,
        channel,
        thread,
      ...
      var parentConverationId = await getParentConversationId(
        conversationId,
        question,
        channel,
        thread,
      ...
      let response = await chatAPI.sendMessage(question, {
        parentMessageId: await getParentConversationId(
          conversationId,
          question,
          channel,
          thread,
      ...
      const message =
        source == Source.Mention
          ? `<@${userId}> You asked: ${shortenText(
              parentConverationId == null
                ? question.substring(botContext.length)
                : question,
              100
            )}\n${response.text}`
          : `${response.text}`;
...
function setParentConversationId(
  conversationId: string,
...
async function getParentConversationId(
  conversationId: string,
  text: string,
  channel: string,
  thread: string,
  ...
  if (userConversation) {
    ...
    if (timeDiff > 1000 * 60 * 30) {
      await say({
        channel,
        thread_ts: thread,
        text: "Starting a new conversation as there was no response in the last 30 minutes.",
      });
      conversations[conversationId] = {
        parentMessageId: undefined,
        timestamp: undefined,
